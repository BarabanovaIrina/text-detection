...
def letter_extraction(image_file: str, out_size=28) -> List[Any]:
    def resize_image(idx, contour, letters):
    ...
    for idx, contour in enumerate(contours):
        t = threading.Thread(target=resize_image, args=(idx, contour, letters))
        threads.append(t)
        t.start()
        t.join()
...
def fit_model(model=None, data=None):
    x_train_cat, y_test_cat, X_train, X_test = data
...
def img_to_str(model: Any, image_file: str):
    letters = letter_extraction(image_file)
    s_out = ""
...
def emnist_data_prepare():
    ...
    return x_train_cat, y_test_cat, X_train, X_test


if __name__ == '__main__':
    # data: tuple = emnist_data_prepare()
    # model = emnist_model()
    # fit_model(model, data)
    # model.save('emnist_letters.h5')

    model = keras.models.load_model('fitted_model.h5')
    s_out = img_to_str(model, "data/hello.png")
    print(s_out)
